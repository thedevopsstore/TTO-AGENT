...
def main() -> None:
    ...
    with ExitStack() as stack:
        clients, registry = open_sessions(settings, stack)
        tools_module.REGISTRY = registry

        if "servicenow" not in clients:
            raise RuntimeError("ServiceNow MCP client is required but unavailable.")

        planner = Agent(
            name="TTO Planner",
            model=settings.model,
            system_prompt=PLANNER_PROMPT,
            tools=[clients["servicenow"], build_tto_task_list],
        )

        runner = Agent(
            name="TTO Runner",
            model=settings.model,
            system_prompt="You hold the workflow tool and the MCP clients needed by the tasks.",
            tools=[*clients.values(), workflow],
        )
        ...
        log.info(
            "Agents built: planner=[servicenow, build_tto_task_list], "
            "runner=[%d MCP clients, workflow], a2a=[validate_tto_checklist]. "
