...
async def list_upstream_tools(settings: Settings) -> list[Tool]:
    """Fetch the upstream tool list using the default credential chain (bootstrap role)."""
...
async def call_upstream_tool(
    settings: Settings,
    creds: CachedCredentials | None,
    tool_name: str,
    arguments: dict[str, Any],
) -> Any:
    """Call one upstream AWS MCP tool. Pass creds=None to use the default credential chain."""
    boto_creds = None
